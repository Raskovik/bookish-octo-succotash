...
import type { PatternFormState } from "./actions";
import type { PatternRow, TraitRarity } from "@/lib/supabase/types";

const TRAIT_RARITIES: TraitRarity[] = ["common", "uncommon", "rare"];
const initialState: PatternFormState = null;
...
export function PatternForm({
  ...
  action: (prevState: PatternFormState, formData: FormData) => Promise<PatternFormState>;
  pattern?: PatternRow;
  breedOptions: { id: string; name: string }[];
  submitLabel: string;
}) {
  const [state, formAction, isPending] = useActionState(action, initialState);
  const [url, setUrl] = useState(pattern?.layer_url ?? "");

  return (
    <form action={formAction} className="flex max-w-md flex-col gap-5">
      {pattern ? <input type="hidden" name="pattern_id" value={pattern.id} /> : null}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-xs text-stone-500">Patterns are breed-specific — the overlay must match that breed&apos;s proportions.</p>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            id="layer_url"
            name="layer_url"
            defaultValue={pattern?.layer_url ?? ""}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <select
          id="rarity_tier"
          name="rarity_tier"
          defaultValue={pattern?.rarity_tier ?? "common"}
          className="rounded-md border border-green-300 px-3 py-2 text-sm dark:border-stone-700 dark:bg-stone-900"
        >
      ...
      <label className="flex items-center gap-2 text-sm font-medium">
        <input
          type="checkbox"
          name="is_active"
          defaultChecked={pattern?.is_active ?? true}
